...
describe("sliceValues", () => {
  ...
  it("should handle an exact match of the entire range", () => {
    expect(test([1, 2, 3], 1, 3)).toEqual([1, 2, 3]);
  });

  it("should handle ranges that are before the sorted values", () => {
    expect(test([2, 3, 4], 0, 1)).toEqual([]);
  });

  it("should handle ranges that are after the sorted values", () => {
    expect(test([2, 3, 4], 5, 6)).toEqual([]);
  });

  it("should handle partially overlapping ranges", () => {
  ...
  it("ranges that are proper subsets", () => {
